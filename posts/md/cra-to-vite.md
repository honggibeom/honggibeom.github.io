...
거기에 CRA는 2023년 이후로 사실상 유지보수가 멈춰 있다. `react-scripts@5`가 물고 있는 webpack 4/5 계열 의존성 때문에 `npm install`을 할 때마다 취약점 경고가 수십 개씩 쏟아졌고, React 19를 쓰면서도 빌드 도구는 몇 년 전에 머물러 있는 상태였다.
...
이 프로젝트는 JSX를 전부 `.jsx`가 아닌 `.js` 확장자로 쓰고 있었다. CRA는 babel로 전부 처리하니까 아무 문제가 없다. 그런데 Vite는 esbuild를 쓰고, **esbuild는 기본적으로 `.js` 파일에 JSX가 들어있을 거라고 가정하지 않는다.**
...
- dev 서버 기동이 수십 초에서 1초 미만으로 줄었다. Vite는 dev에서 번들링을 하지 않고 브라우저의 네이티브 ESM을 그대로 쓴다.
