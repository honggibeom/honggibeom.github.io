---
title: StockAdvisor 보안과 설정 - 시크릿 분리, 관리자 승격, 배치 API 키, 권한 체계
date: 2026-09-02
category: stockadvisor
src: cover.svg
tags: [stockadvisor, spring-security, spring-boot, 보안, oauth2, 설정관리]
summary: StockAdvisor 보안과 설정 기록. 평문으로 있던 API 키·DB 비밀번호를 application-secret.yml로 분리하고 optional 없이 import해 없으면 기동이 실패하게, 소셜 로그인 전용에서 관리자를 이메일 등록 + 로그인 시 승격으로, permitAll이던 배치 엔드포인트에 X-Batch-Key, 자원별 권한 체계.
---
...
변곡점 신호를 넣는 `POST /stocknews`가 `permitAll`이라 누구나 가짜 신호를 넣을 수 있었다. 48자 랜덤 공유 키를 `X-Batch-Key` 헤더로 검사하고, 파이썬이 `.env`의 `BATCH_API_KEY`를 보낸다. 사용자 인증과는 별개 경로다. 배치는 로그인할 수 없으니까.
