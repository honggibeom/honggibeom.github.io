...
여기서 배운 건, **필터는 "필터 하나"가 아니라 "독립적인 술어(predicate)의 모음"으로 봐야 한다**는 것이다. 처음부터 그렇게 봤다면 리팩터링할 일도 없었을 것이다.
...
토스페이먼츠 SDK를 붙였다. 결제 자체보다 어려웠던 건 **성공과 실패 이후의 흐름**이었다.
...
nearby는 기능 목록으로 보면 평범한 서비스다. 지도, 리스트, 필터, 예매, 결제. 하지만 이 다섯 개를 **하나의 흐름으로 이어붙이는 일**이 각각을 만드는 것보다 훨씬 어려웠다.
