---
title: 오라클 DB 학습 노트 (1) - 구조와 기본기, 그리고 DDL
date: 2025-07-18
category: oracle
src: cover.svg
tags: [oracle, database, sql, ddl, 학습노트]
summary: 오라클을 처음부터 다시 정리하는 8부작 시리즈의 첫 글. 전체 학습 로드맵과 실습 환경, 인스턴스·테이블스페이스 구조, 데이터 타입, 테이블과 제약조건까지 다룬다.
---
...
- **SGA (System Global Area)** — 인스턴스 전체가 공유한다.
  ...
  - *공유 풀*: 파싱한 SQL과 실행계획을 캐시한다. **바인드 변수를 써야 하는 이유**가 여기 있다. 리터럴을 그대로 박은 SQL은 값마다 다른 SQL로 인식돼 매번 하드 파싱이 일어나고 공유 풀을 오염시킨다.
...
컬럼 길이를 **줄이는 것**은 그 컬럼에 데이터가 없거나 전부 새 길이 안에 들어갈 때만 된다.
