---
title: StockAdvisor 변곡점 예측 - Keras 앙상블, 임계값 이진 탐색, 종목당 1분을 밀리초로
date: 2026-09-02
category: stockadvisor
src: cover.svg
tags: [stockadvisor, python, tensorflow, keras, 변곡점, 기술적분석]
summary: StockAdvisor 변곡점 예측 모델 기록. 40일 기술지표 창과 Keras 3개 앙상블, ZigZag 정답, 고점·저점을 따로 정하는 임계값 이진 탐색과 4단계 노이즈 필터, 그리고 종목당 1분 걸리던 예측을 64종목 묶음으로 바꿔 호출을 15,000번에서 240번으로 줄인 과정. (2026-09에 이 배치 구조는 삭제하고 온디맨드로 전환했다 — 후속 글 참고)
---
...
> **2026-09 업데이트** — 이 글에 나오는 배치 예측(파이프라인의 predict 단계, `predict_fast.py`)은 지금은 없다. "AI 가격예측" 버튼을 눌렀을 때 Spring 백엔드가 계산하는 온디맨드 구조로 바꾸고, 모델도 PyTorch로 다시 학습한 v2로 교체했다. 아래 임계값 이진 탐색에도 상한 규칙이 하나 추가됐다. 무엇을 왜 바꿨는지는 [변곡점 예측 v2 - 배치를 지우고 온디맨드로](/post/stockadvisor-turning-point-v2)에 있다. 이 글은 그 전까지의 기록으로 남겨 둔다.
...
Keras 모델 3개 앙상블이다. 40일 창의 기술지표로 시퀀스를 만들고 고점/저점/보통 3분류 확률을 낸다. 정답은 ZigZag(5%)로 만든다. ZigZag는 직전 전환점에서 5% 이상 반대로 움직여야 새 고점·저점으로 인정하는 필터라, 잔파동을 걸러낸 "진짜 꺾인 자리"가 정답이 된다.
...
여기엔 나중에 드러난 함정이 있다. 신호는 최소 10일 간격을 지켜야 해서 **나올 수 있는 봉우리 수에 상한**이 있는데, 변동성이 큰 국내 종목은 목표(극점 × 1.8)가 그 상한을 넘어버린다. 그러면 이진 탐색이 하한까지 내려가 임계값이 0.01이 된다. 지금은 목표를 `전체 봉우리 수 × 0.8`로 자른다 — [v2 글](/post/stockadvisor-turning-point-v2)에 자세히 적었다.
...
그리고 이 구조는 오래가지 않았다. 계산을 싸게 만들긴 했지만 **애초에 전 종목을 미리 계산할 이유가 없었다.** 사용자가 실제로 열어보는 종목은 하루에 수십 개인데 5,000종목을 20분마다 다시 돌리고 있었다. 결국 배치를 통째로 지우고 버튼을 눌렀을 때만 계산하는 구조로 옮겼다 → [변곡점 예측 v2](/post/stockadvisor-turning-point-v2).
