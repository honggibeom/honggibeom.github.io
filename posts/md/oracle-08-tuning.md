...
[1편](/post/oracle-01-basics)에서 본 것처럼 오라클의 I/O 단위는 블록이다. 100만 건 테이블에서 한 건을 찾을 때, 인덱스가 없으면 모든 블록을 읽어야 한다. 인덱스가 있으면 몇 개 블록만 읽고 끝난다.

오라클의 기본 인덱스는 B\*Tree다. 구조는 이렇다.
...
| `INDEX FAST FULL SCAN` | 인덱스 전체를 순서 무시하고 멀티블록 I/O로 |
...
1. `=` 조건으로 쓰이는 컬럼을 앞에, 범위(`>`, `BETWEEN`, `LIKE`) 조건은 뒤에 둔다. 범위 조건이 앞에 오면 그 뒤 컬럼은 스캔 범위를 좁히는 데(`access`) 못 쓰이고 `filter`로만 쓰인다.
2. 여러 쿼리에서 공통으로 쓰이는 컬럼을 앞에 둔다.
3. 값의 종류가 많아 선택도가 좋은 컬럼을 앞에 두는 게 유리한 경우가 많지만, 1번이 우선이다.
...
함수를 꼭 써야 한다면 함수 기반 인덱스를 만든다. 단, 쿼리의 표현식과 인덱스 정의가 정확히 일치해야 한다.
...
[2편](/post/oracle-02-select-functions)에서 본 암시적 형변환이 여기서 성능 문제로 돌아온다. 문자 컬럼을 숫자와 비교하면 오라클이 컬럼 쪽에 `TO_NUMBER`를 씌운다. 결과적으로 1번과 같은 상황이 된다.
...
`E-Rows`(예상 행 수)와 `A-Rows`(실제 행 수)를 나란히 볼 수 있다. 이 둘이 크게 어긋나는 지점이 문제의 출발점이다. 대개 통계정보가 낡았거나 조건식이 옵티마이저를 헷갈리게 만든 것이다.
...
오라클은 기본적으로 자동 통계 수집 작업을 밤에 돌린다. 하지만 대량 적재 직후처럼 데이터가 급변한 경우에는 직접 수집해야 한다. "어제까지 잘 돌던 쿼리가 갑자기 느려졌다"의 상당수가 통계 문제다.
