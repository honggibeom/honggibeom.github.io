...
src: cover.svg
tags: [stockanalyst, spring, onnx, 동시성, 세마포어, 스레드풀, k6, 부하테스트, 최적화]
summary: 뉴스 감성 추론이 요청 스레드 위에서 제한 없이 돌고, 검색 요청마다 og:image 스레드풀을 새로 만들었다 버리고 있었다. ONNX 추론은 Semaphore(2)로 자리를 정하고 못 잡으면 감성 없이 응답하게, 이미지 풀은 공용 고정 풀 하나로 바꿨다. 그 뒤 k6 로 50VU 씩 400 까지 올려 보니 200VU 부터 502 가 났는데, 범인은 백엔드가 아니라 nginx 의 임시 포트 고갈이었다. keepalive 세 줄로 고친 뒤 400VU 까지 실패 0, 그래서 몇 명까지 받을 수 있는지까지.
---
...
- 고친 뒤 k6 로 세 화면을 섞어 10, 20, 40VU 순서로 4분: 11,507요청, 실패 0, 요청 p95 32ms, 페이지 묶음 p95 79ms. 서버는 한계 근처에도 안 갔다. 상한은 50VU 씩 올려서 따로 찾았고, 그 얘기는 다음 글에 있다
...
고치고 나서 부하를 걸어 봤다. 도구는 k6 하나, 스크립트 하나(`loadtest/stock-load.js`)다. 사용자 행동을 세 가지로 흉내 냈다. 홈(지수·환율·순위·뉴스·의견 변경·다우이론, 7요청 묶음) 40%, 종목 상세(캔들 1만 행을 받은 뒤 나머지 11~13요청 묶음, 국내·해외 절반씩) 45%, 공매도 화면(시장 순위 + 미국 순위) 15%. 각 VU 는 화면을 열고 1~5초 생각한 뒤 다음 화면으로 간다. 단계는 10VU 1분, 20VU 1분, 40VU 30초로 총 4분.

테스트 전에 두 가지를 정리하려 했다. 즉석 기사 검색은 부하 테스트가 그대로 네이버 호출이 되니 `SHORT_NEWS_LIVE_MAX=0` 으로 끄려고 `.env` 에 넣었는데, 나중에 보니 compose 가 그 변수를 컨테이너로 넘기지 않아 기본값 12 로 켜진 채였다. DB 에 기사가 있는 종목은 안 부르니 실제 네이버 호출은 첫 몇 번뿐이었지만, "껐다"고 믿고 돌린 건 잘못이다 — 끈 줄 알았던 설정은 컨테이너 안에서 `env` 로 확인해야 한다. 그리고 온디맨드 예측(`/stocknews/predict`)은 아예 안 부른다. 종목당 수천 시퀀스를 도는 요청이라 부하 테스트에 넣으면 그게 병목의 전부가 되어 나머지가 안 보인다.

### 결과

| 항목 | 값 |
|---|---|
| 총 요청 | 11,507건 (평균 47/s, 40VU 구간 약 100/s) |
| 실패 | 0건 |
| 요청 지연 | 중앙값 5ms · p95 32ms · p99 41ms |
| 홈 묶음 | p95 9ms (최대 1.8초, 첫 요청이 KIS 순위·야후 지수를 실제로 받아 온 순간) |
| 종목 상세 묶음 | p95 30ms, 캔들 1만 행 조회 p95 40ms |
| 공매도 화면 | p95 45ms · 최대 753ms |
| 페이지 한 장(묶음 전체) | p95 79ms |

<svg viewBox="0 0 760 170" width="100%" style="max-width:760px;display:block;margin:24px auto" role="img" aria-label="화면별 p95 지연 막대 — 홈 9ms, 종목 상세 30ms, 캔들 40ms, 공매도 45ms, 페이지 묶음 79ms, 목표선 1500ms 는 화면 밖">
  <g font-size="12" fill="var(--color-text)">
    <text x="8" y="30">홈</text><rect x="120" y="18" width="18" height="16" rx="3" fill="var(--color-accent)"/><text x="146" y="30" fill="var(--color-text-dim)">9ms</text>
    <text x="8" y="60">종목 상세</text><rect x="120" y="48" width="60" height="16" rx="3" fill="var(--color-accent)"/><text x="188" y="60" fill="var(--color-text-dim)">30ms</text>
    <text x="8" y="90">캔들 1만 행</text><rect x="120" y="78" width="80" height="16" rx="3" fill="var(--color-accent)"/><text x="208" y="90" fill="var(--color-text-dim)">40ms</text>
    <text x="8" y="120">공매도</text><rect x="120" y="108" width="90" height="16" rx="3" fill="var(--color-accent)"/><text x="218" y="120" fill="var(--color-text-dim)">45ms</text>
    <text x="8" y="150">페이지 묶음</text><rect x="120" y="138" width="158" height="16" rx="3" fill="var(--color-accent-soft)" stroke="var(--color-accent)"/><text x="286" y="150" fill="var(--color-text-dim)">79ms</text>
    <text x="560" y="30" fill="var(--color-text-dim)" font-size="11">막대 2px = 1ms · 목표 p95 1,500ms 는</text>
    <text x="560" y="46" fill="var(--color-text-dim)" font-size="11">이 그림 폭의 4배 오른쪽에 있다</text>
  </g>
</svg>

목표로 잡았던 홈 1.5초·상세 2.5초와 비교하면 자릿수가 두 개 낮다. 40명이 동시에 계속 클릭하는 동안 서버는 사실상 놀았다.

### 이 숫자가 말하는 것과 말하지 않는 것

이 결과만으로 말할 수 있는 건 "40명 동시 사용에서 어떤 열화도 없다"까지다. 대기가 생기면 p95 가 중앙값에서 멀어지기 시작하는데(중앙값 5ms 대 p95 32ms 는 정상 분산 범위다), 그 조짐이 없었다. 상한은 올려 봐야 나온다.

### 상한은 따로 찾았다

40VU 는 기준선일 뿐이라, 이어서 50VU 씩 400 까지 올려 봤다. 150VU 까지는 실패 0 이었고 200VU 부터 502 가 나기 시작했는데, 범인은 세마포어도 Tomcat 도 아니고 nginx 가 요청마다 백엔드 연결을 새로 열고 닫아서 임시 포트가 바닥난 것이었다. 그 추적 과정과 keepalive 세 줄로 고친 뒤의 수치는 [부하테스트 글](/post/stockanalyst-loadtest-port-leak)에 따로 적었다. 고친 뒤 기준으로 p95 가 100ms 안에 머무는 200VU 는 동시 접속자 1,500명 안팎에 해당하고, 300VU 부터는 CPU 가 벽이다.

세마포어 쪽은 이 테스트로 "값이 맞다"고 말할 수 없다. 40VU 에서도 감성 추론이 3초 안에 자리를 못 잡아 건너뛴 로그(`감성 추론 대기 초과`)를 세어 봐야 하는데, 이 테스트는 즉석 기사 검색을 끈 상태라 추론 경로에 부하가 거의 안 걸렸다. 추론 세마포어가 실제로 줄을 세우는지는 즉석 검색을 켠 채로 공매도 화면만 집중해서 때리는 두 번째 테스트가 필요하다. 지금 확인된 건 "세마포어를 넣어서 느려진 곳은 없다"는 것뿐이다.

스크립트 쪽에서도 배운 게 있다. 처음 돌렸을 때 실패율이 5.2% 였는데 전부 뉴스 검색 한 곳이었고, 원인은 k6 가 URL 의 한글 검색어를 인코딩하지 않아 톰캣이 400 으로 거절한 것이었다. 서버 문제가 아니라 스크립트 문제였는데, 요약에 어느 요청이 어떤 상태코드로 실패했는지를 찍지 않았으면 한참 헤맸을 것이다. 스크립트를 어떻게 고쳐 썼는지는 [k6 스크립트 글](/post/stockanalyst-k6-script-tips)에 정리했다.

## 6. 남은 것

`max-concurrent=2`, `acquire-timeout=3000ms`, `pool-size=12` 세 숫자는 여전히 감으로 정한 값이다. 이번 테스트가 준 건 "이 값으로 150VU 까지 문제없다"는 하한이지 근거가 아니다. 세마포어가 실제로 줄을 세우는 순간은 즉석 검색을 켠 채 뉴스 검색을 서로 다른 검색어로 때려야 보인다, 같은 검색어는 감성이 DB 에 이미 있어 추론을 안 탄다. 그래서 대기 초과를 debug 로그 대신 누적 카운터(건너뛴 요청 수·기사 수·추론 완료 수)로 세고 10초에 한 번 INFO 로 찍게 바꿨다. 이게 있어야 운영에서 "감성이 비어서 온 응답이 하루에 몇 건인지"를 알 수 있다.
