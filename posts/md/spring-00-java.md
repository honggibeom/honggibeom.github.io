---
title: Spring 전에 알아야 할 자바와 객체지향 - 인터페이스, 예외, 프록시 (Spring 로드맵 0)
date: 2026-03-26
category: spring
src: cover.svg
tags: [spring, java, 객체지향, 로드맵, 백엔드]
summary: Spring을 이해하는 데 실제로 쓰이는 자바 지식만 골랐다. 인터페이스 의존, 예외 체계, 프록시 패턴, 리플렉션과 애노테이션. Spring이 마법처럼 보이는 이유는 대부분 자바 쪽이 비어 있어서다.
---

> Spring 공부 로드맵 시리즈 0편. 전체 목차는 [로드맵 글](/post/spring-roadmap)에 있다. 아직 Spring은 한 줄도 나오지 않는다. 뒤에서 겪을 문제 대부분이 실은 자바 쪽에서 갈리기 때문에, 그 바닥부터 먼저 깐다.
