...
> 도커 학습 노트 시리즈 8편(마지막). [7편](/post/docker-07-compose)에서 여러 컨테이너를 Compose로 묶는 데까지 왔다. 이번에는 이미지를 작게 만들고, 안전하게 굴리고, 문제가 났을 때 원인을 좁혀 들어가는 방법이다.

## 이미지가 큰 게 왜 문제인가

"돌아만 가면 되지"로 넘기기 쉽지만, 이미지 크기는 여러 군데에서 비용으로 돌아온다.
...
크기의 대부분은 빌드에만 필요했던 것들이다. JDK, `node_modules`의 devDependencies, gcc, 소스 코드, git 히스토리. 실행에는 하나도 필요 없다.
...
빌드 결과물은 정적 파일뿐이므로 최종 이미지에 Node가 아예 없다. 1.2GB → 50MB 수준으로 줄어든다.
...
`CGO_ENABLED=0`으로 정적 링크해야 `scratch`나 `distroless/static`에서 돈다. `-ldflags="-s -w"`는 디버그 심볼을 빼서 바이너리를 줄인다. 최종 이미지가 10MB 안팎이 된다.
...
컨테이너 보안은 [1편](/post/docker-01-container-basics)에서 본 "격리는 커널 경계일 뿐"이라는 사실에서 출발한다. 경계가 얇으니 그 위에 얹는 것을 최소로 줄이는 게 전부다. 순서대로 다섯 가지다.
...
- `--read-only` + `tmpfs`는 [5편](/post/docker-05-volume-data)에서 본 구성이다
...
`docker.sock` 마운트는 CI 러너나 Watchtower 같은 도구가 흔히 요구한다. 그 컨테이너가 침해되면 호스트 전체가 넘어간다는 걸 이해하고 쓰거나, 소켓 프록시(`tecnativa/docker-socket-proxy`)로 허용 API를 제한한다.

### 4. 시크릿을 이미지에 넣지 않는다

[2편](/post/docker-02-image-registry)과 [4편](/post/docker-04-dockerfile)에서 본 대로, 레이어에 한 번 들어간 것은 지워도 남는다.
...
실무에서 가장 흔한 절충은 비밀 파일을 읽기 전용으로 마운트하고 애플리케이션이 파일에서 읽게 하는 것이다.
...
실수로 커밋한 뒤라면 이미지를 다시 빌드하는 것만으로는 부족하다. 이미 push 된 이미지를 지우고, 노출된 자격 증명을 폐기하고 재발급해야 한다.
...
다만 순서가 중요하다. 여기까지의 내용(이미지, 볼륨, 네트워크, 프로세스와 시그널)을 모른 채 쿠버네티스로 가면 전부 마법으로 보인다. 파드는 컨테이너의 묶음이고, 퍼시스턴트 볼륨은 5편의 볼륨이고, 서비스는 [6편](/post/docker-06-network)의 DNS다. 개념이 그대로 확장된다.
