...
이 테이블을 그대로 다시 쓰는 화면이 나중에 생긴다. [국채·금리 페이지](/post/stockanalyst-dev-log-3-signals-rates)다. 같은 지표를 실시간 시세로 따로 받는 [환율 위젯](/post/stockanalyst-market-widgets)은 별개 경로다. 그리고 여기 적힌 "한국 기준금리"는 사실 재할인율이었다는 것도 그때 드러났다.

## 변곡점 예측 모델

Keras 모델 3개 앙상블이다. 40일 창의 기술지표로 시퀀스를 만들고 고점/저점/보통 3분류 확률을 낸다. 정답은 ZigZag(5%)로 만든다. ZigZag는 직전 전환점에서 5% 이상 반대로 움직여야 새 고점·저점으로 인정하는 필터라, 잔파동을 걸러낸 "진짜 꺾인 자리"가 정답이 된다. 같은 필터를 [다우 이론 위젯](/post/stockanalyst-market-widgets)에서도 쓴다 — 쓰임은 다르지만 "어디가 스윙 고저점인가"를 정하는 도구는 하나다.
