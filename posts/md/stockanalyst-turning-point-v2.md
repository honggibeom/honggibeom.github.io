...
이 숙제는 문구를 고치는 대신 **신호 자체를 두 종류로 나누는 것**으로 풀었다. 그날까지의 데이터만 보고 찍혀 다시는 안 바뀌는 신호와, 전 구간을 다시 보고 찍는 사후 분석을 차트에서 모양으로 구분한다 → [개발기 (3)](/post/stockanalyst-dev-log-3-signals-rates).
